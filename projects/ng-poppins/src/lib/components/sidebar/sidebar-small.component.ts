...
import { slideInLeftFactory, slideDownFactory } from '../../animations';
import { CSS_CLASS_NAME_PREFIX } from '../comp-utils';
const slideInLeft = slideInLeftFactory();
const slideInLeft2 = slideInLeftFactory( 'slideInLeft2', '.3s ease-in-out', '-20px' );
const slideDown = slideDownFactory( 'slideDown', '.5s 0.9s ease-in-out' );
@Component( {
  animations: [slideInLeft, slideInLeft2, slideDown],
...
export class NgPopSidebarSmallComponent implements OnInit
{
  ...
  @Input() menus: Menu[];
  @Output() readonly ev = new EventEmitter();
  @Input() width = '50px';
